...
bool JumpingRB = false;

RBT buscarRB(RBT nodoRBT, int dato)
{
   RBT actual = nodoRBT;
   if(nodoRBT==NULL){
   ...
   while(actual!= NULL)
   {
      ...
      {
        if(JumpingRB) {
            cout<<"\n\tThe item "<<dato<<" exit in the tree\n";
        }
   ...
   if(JumpingRB) {
        cout<<"\n\tThe item "<<dato<<" not exit in the treeol\n";
   }

   return NULL;
}

void verArbolRB(RBT arbol, int n)
{
     if(arbol==NULL)
        return;
     verArbolRB(arbol->der, n+1);
...
void insertarRB(RBT &arbol, int dato)
{
   RBT padre = NULL;
   RBT actual = arbol;
...
void eliminarRB (RBT &arbol, int x)
{
    RBT aux1,aux2;
    if(arbol==NULL)
        return;
    if(x<arbol->nro)
        eliminarRB(arbol->izq,x);
    if(x>arbol->nro)
        eliminarRB(arbol->der,x);
    if(x==arbol->nro)
    {
